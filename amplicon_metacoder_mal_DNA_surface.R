...
tr<-c(1:120)  # names(otu1)
dr<-c(121:length(names(otu1)))
otu2<-otu1 %>% 
...
dr<-c(1:17,207:222)   ## rows of data for sam    names(sdat)
...
#################################
names(otu4)
tr<-c(2:118)  # rows of samples taxon col 1
...
sam<- sam %>% 
  arrange(factor(sst_mean_cat, levels=c("<20",">20 and <25",">25"))) 
