...
x<-sdat$rich   # log best  profile only-log
...
qqline(resid(mod))
###################  test interactions
#           #   summary(fit.errdurb, Nagelkerke=T)  summary(mod)
### check if interactions improve model
mod2<-lm(log(rich)~(sDepth+sLand_Dist+sPresent.Surface.Temperature.Mean+
                 sPresent.Surface.Temperature.Range+sPresent.Surface.Primary.productivity.Mean+
...
fit.durb<-errorsarlm(mod2, ssdat, listw=us.wt4, etype="emixed", method="MC")
summary(fit.durb, Nagelkerke=T)
AIC(mod2)   ## 210
Anova(mod2)
