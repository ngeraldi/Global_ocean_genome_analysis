...
#library(MASS)
library(car)
#  rare or raw reads  .  otu or species.   DNA or RNA      low(90% pid) or high filter (97%)
...
AICs
#   AIC(fit.errdurb) #  best119  r2 72
##check assumptions        summary(mod)  summary(lmer.mod)
mod<-mod2
hist(resid(mod))
...
summary(mod)
###################  test interactions
#         for richnes sma model best 214    
...
mod2<-errorsarlm(mod_in, ssdat, listw=us.wt4, etype="emixed", method="MC")
summary(mod_in)

summary(mod2, Nagelkerke=T)
...
AICs<-c(AIC(lmer.mod),AIC(mod),AIC(fit.lag), AIC(fit.err), AIC(fit.durb), AIC(fit.errdurb), AIC(fit.sac), AIC(fit.sma))
AICs   # fit.errdurb)  284
###################  test interactions
# for reads fit.durb best 300  included interactions higer AIC
...
mod_in<-lm(log(reads+1)~sDepth+sLand_Dist+sPresent.Surface.Temperature.Mean+
           ...
           #+sLand_Dist:sPresent.Surface.Primary.productivity.Mean
           #+sLand_Dist:sPresent.Surface.Primary.productivity.Range 
           #+sLand_Dist:sOHI_2013 
           +sPresent.Surface.Temperature.Mean:sPresent.Surface.Temperature.Range
           
             , ssdat)
fit.errdurb<-errorsarlm(mod_in, ssdat, listw=us.wt4, etype="emixed", method="MC")
#fit.durb<-errorsarlm(mod_in, ssdat, listw=us.wt4, etype="emixed", method="MC")
summary(fit.errdurb, Nagelkerke=T)
...
AIC(mod_in)
########################################################################################################
###  diversity          x<-sdat$shan    # log
lmer.mod<-lmer(log(shan+1)~sDepth+sLand_Dist+sPresent.Surface.Temperature.Mean+
                 sPresent.Surface.Temperature.Range+sPresent.Surface.Primary.productivity.Mean+
                 sPresent.Surface.Primary.productivity.Range+sOHI_2013+(1|lohghurst_biome/Station), data=sdat)###
#
#   mes<- c(AIC(lmer.mod),AIC(lmer.mod2))
mod<-lm(log(shan+1)~sDepth+sLand_Dist+sPresent.Surface.Temperature.Mean+
          sPresent.Surface.Temperature.Range+sPresent.Surface.Primary.productivity.Mean+
          sPresent.Surface.Primary.productivity.Range+sOHI_2013,ssdat)
#SAR - Lag model
fit.lag<-lagsarlm(mod, ssdat, listw=us.wt4, type="lag", method="MC")
...
AICs
###################  test interactions
# for div fit.durb) best 21 AIC and r2 .06
### check if interactions improve model
mod_in<-lm(log(shan+1)~(sDepth+sLand_Dist+sPresent.Surface.Temperature.Mean+
                  sPresent.Surface.Temperature.Range+sPresent.Surface.Primary.productivity.Mean+
                  sPresent.Surface.Primary.productivity.Range+sOHI_2013)^2,ssdat)
mod_in<-lm(log(shan+1)~sDepth+sLand_Dist+sPresent.Surface.Temperature.Mean+
                          sPresent.Surface.Temperature.Range+sPresent.Surface.Primary.productivity.Mean+
