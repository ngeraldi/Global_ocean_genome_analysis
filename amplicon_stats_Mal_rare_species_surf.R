...
lmer.mod<-lmer(log(rich+1)~sDepth+sLand_Dist+sPresent.Surface.Temperature.Mean+
...
mod<-lm(log(rich+1)~sDepth+sLand_Dist+sPresent.Surface.Temperature.Mean+
          sPresent.Surface.Temperature.Range+sPresent.Surface.Primary.productivity.Mean+
          sPresent.Surface.Primary.productivity.Range+sOHI_2013,ssdat) # summary(mod)
#SAR - Lag model
...
mod2<-lm(log(rich+1)~(sDepth+sLand_Dist+sPresent.Surface.Temperature.Mean+
                 sPresent.Surface.Temperature.Range+sPresent.Surface.Primary.productivity.Mean+
                 sPresent.Surface.Primary.productivity.Range+sOHI_2013)^2,data=sdat)##
mod2<-lm(log(rich+1)~sDepth+sLand_Dist+sPresent.Surface.Temperature.Mean+
                         sPresent.Surface.Temperature.Range+sPresent.Surface.Primary.productivity.Mean+
                         sPresent.Surface.Primary.productivity.Range+sOHI_2013
         
           ,data=sdat)
fit.errdurb<-errorsarlm(mod2, ssdat, listw=us.wt4, etype="emixed", method="MC")
summary(fit.errdurb, Nagelkerke=T)
AIC(mod2)   ## 210
Anova(mod2)
#  chekc interaction
jtools::interact_plot(mod2, pred = "sPresent.Surface.Primary.productivity", modx = "sLand_Dist", 
