...
```{r define_universal}
#stud_pat<-""  # matches study specific title from pipe (begining of files).
...
geo<-read.table(paste(dir,"/Documents/KAUST/eDNA/DMAP/R/CSV/Global_layers_genome.csv",sep=""), sep=",",header=T)
#  names(geo)
geo2<- geo %>% 
  dplyr::select(Latitude:ocean) %>% 
  distinct(Latitude,Latitude, .keep_all = T)

amplicon_geo<-read.table(paste(dir,"Documents/KAUST/eDNA/DMAP/R/CSV/Global_layers_oct18.csv",sep=""), sep=",",header=T)
...
## ampli TARA tidy
```{r TARA t}
source(paste(dir,"Documents/KAUST/eDNA/R/projects/Global_ocean_genome_analysis/amplicon_stats_tara_source.R",sep=""))
...
#  print(p2)
## save  
ggsave(filename=paste(dir,"/Documents/KAUST/eDNA/DMAP/R/plots/metagen_and_18s/env_sig_var_10_plot.pdf",sep="")
        , plot = p, width = 20, height = 26, units = "cm" )

```

## plot latitude
uses 4 plots from above
```{r plot prof}
...
## save  
ggsave(filename=paste(dir,"Documents/KAUST/eDNA/DMAP/R/plots/metagen_and_18s/lat_plot.pdf",sep="")
...
p<- ggpubr::ggarrange(tp, mpp, dpp, #+ ggpubr::rremove("x.text"), 
  ...
  print(p)
## save  
ggsave(filename=paste(dir,"Documents/KAUST/eDNA/DMAP/R/plots/comparison_2019/rich_abund_3cruises.pdf", sep="")
