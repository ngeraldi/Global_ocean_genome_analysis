---
title: "amplicon_richness_environmental_var"
...
```{r}
#    names(dat)
tp<- t_rare %>% # names(t_all)   names(t_rare)
  dplyr::select(Depth,land_dist,Present.Surface.Temperature.Mean,Present.Surface.Temperature.Range,
         Present.Surface.Primary.productivity.Mean, Present.Surface.Primary.productivity.Range,OHI_2013, filter_size_cat,
         seq,reads,rich,shan,Sample.ID,Locality,Locality.2,Locality.4, Latitude, Longitude)  
...
datp<- sdat %>% 
  select(sPresent.Surface.Temperature.Range, Latitude, Longitude,rich,Locality.2) %>% 
  mutate(Latitude=as.integer(Latitude/10), Longitude=as.integer(Longitude/10)) %>% 
  group_by(Locality.2) %>%   #, Latitude, Longitude
  summarise(rich=mean(rich),temp=mean(sPresent.Surface.Temperature.Range), n())  %>% 
  arrange(temp) %>% 
#filter(!Locality.2=="(MEDI) Mediterranean Sea, Black Sea Province [MRGID:21465]") 

mes<- sdat %>% 
  select(sPresent.Surface.Temperature.Range, Latitude, Longitude,rich,Locality.2) %>% 
  filter(Locality.2=="(MEDI) Mediterranean Sea, Black Sea Province [MRGID:21465]" | Locality.2=="(REDS) Red Sea, Persian Gulf Province [MRGID:21474]" ) %>% 
    group_by(Locality.2) %>%   #, Latitude, Longitude
  summarise(rich=mean(rich),temp=mean(sPresent.Surface.Temperature.Range), n()) 
...
datp<- sdat %>% 
  select(sPresent.Surface.Temperature.Range, Latitude, Longitude,rich,Locality.2) %>% 
  #filter(!(Locality.2=="(MEDI) Mediterranean Sea, Black Sea Province [MRGID:21465]" | Locality.2=="(REDS) Red Sea, Persian Gulf Province [MRGID:21474]" )) %>% 
  mutate(temp=sPresent.Surface.Temperature.Range) %>% 
  bind_rows(mes)
  

#datp<-datp[1:13,]
  ggplot(datp, aes(x = temp, y = log(rich))) +
      geom_point() +
    geom_smooth(se = T, method = "gam") +   # "gam"  "loess"
  labs(y = "Richness", x = "SST range")  +
