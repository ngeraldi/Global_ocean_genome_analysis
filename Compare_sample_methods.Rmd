...
```{r define_universal}
stud_pat<-"Dammam"  # matches study specific title from pipe (beginning of files).
dir<-"/Users/nathangeraldi/Dropbox/"
out_file<-"Documents/KAUST/eDNA/R/pipe_summary"
# export  to project folder
export_cor_plot<-"Documents/KAUST/eDNA/DMAP/R/plots/comparison_2019/"
...
mes<- dat_genome %>% 
 # filter(filter_size_cat=="0.2-3")
  distinct(Phylum)  # Hemichordata , Chaetognatha
```
...
source(paste(dir,"/Documents/KAUST/eDNA/R/projects/Global_ocean_genome_analysis/amplicon_stats_mal_source.R",sep="")  )
names(dat)
...
#  export if needed 
#write.csv(mdat1,file=paste(dir,"Documents/KAUST/eDNA/DMAP/R/CSV/malaspina_zooplankton_sp.csv",sep="") ) 
#
...
#mal_net_bio<-read.csv(paste(dir,"Global_databases/DMAP/tow_biomassZooplankton #Biomass Malaspina.csv",sep="" ))

# abundance from Mariluz
mal_net_abun<-openxlsx::read.xlsx(xlsxFile=paste(dir,"Global_databases/DMAP/tow_biomass/Zooplankton_Malaspina_abund.xlsx",sep=""), sheet=1)
mal_net_abun_depth_convert<-openxlsx::read.xlsx(xlsxFile=paste(dir,"Global_databases/DMAP/tow_biomass/Zooplankton_Malaspina_abund.xlsx",sep="") , sheet=2)

## biomass from Santiago, this sheet also has abundance
mal_net_biomass<-openxlsx::read.xlsx(xlsxFile=paste(dir,"/Global_databases/DMAP/tow_biomass/tow_biomass_ABDCIA_all_MALASPINA.xlsx",sep=""), sheet=1)
...
select_at(c(4:15,20)) %>% 
mutate_all(~replace(., is.na(.), 0)) #  %>% 
#  mutate_all(trans)  # removed trans and now used spearman rank
# pearson default, kendall and spearman rank
corDNA <- cor(DNAcor, method = "spearman")

corrplot(corDNA, type = "lower", order = "hclust", tl.col = "black", 
         tl.srt = 45, addCoef.col="grey20", number.cex = .6,
         col = bb, diag=FALSE)
#  !!!  only 10 samples !!!, but metagenome only has only 13 !!
...
#mutate_all(trans)
# perason default, kendall and spearman rank
cor18abun <- cor(ab_18_cor, method = "spearman")

corrplot(cor18abun, type = "lower", order = "hclust", tl.col = "black", 
         tl.srt = 45, addCoef.col="grey20", number.cex = .6,
         col = bb, diag=FALSE)
#  save    correlation_18sabund_pearson_percent_log
pdf(paste(dir, export_cor_plot,'correlation_18s_abund_spearman.pdf',sep = ""), width=11, height=10 )
corrplot(cor18abun, type = "lower", order = "hclust", tl.col = "black", 
         tl.srt = 45, addCoef.col="grey20", number.cex = .6,
         col = bb, diag=FALSE)
dev.off()
...
## metageno and abund    13 samples
ab_metag_cor<-dat_genome2_ab2 %>%   # names(dat_genome2_ab2)
  filter(filter_size=="0.2.3") %>%
select_at(c(5:13,18)) %>% 
mutate_all(~replace(., is.na(.), 0)) # %>% 
#mutate_all(trans)  # log((x*100)+1)

# perason default, kendall and spearman rank
corgenabun <- cor(ab_metag_cor, method = "spearman")

corrplot(corgenabun, type = "lower", order = "hclust", tl.col = "black", 
         tl.srt = 45, addCoef.col="grey20", number.cex = .6,
         col = bb, diag=FALSE)
#  save    correlation_genome_abund_pearson_percent_log

pdf(paste(dir, export_cor_plot,'correlation_genome_abund_spearman_smallfilt.pdf',sep = ""), width=11, height=10 )
corrplot(corgenabun, type = "lower", order = "hclust", tl.col = "black", 
         tl.srt = 45, addCoef.col="grey20", number.cex = .6,
         col = bb, diag=FALSE)
dev.off()
...
## r2
corrplot(corDNA, type = "lower", order = "hclust", tl.col = "black", 
         tl.srt = 45, cl.lim=c(-1,1), addCoef.col="grey20", number.cex = .6,
         diag=FALSE)
#  !!!  only 6 samples !!! - pf7_50 best! .71, better than net at .48, ec4_70 0.2
#  save    correlation_DNAnet_pearson_percent_log
pdf(paste(dir, export_cor_plot,'correlation_DNA_net_spearman.pdf',sep = ""), width=11, height=10 )
corrplot(corDNA, type = "lower", order = "hclust", tl.col = "black", 
         tl.srt = 45, addCoef.col="grey20", number.cex = .6,
         col = bb, diag=FALSE)
dev.off()
...
mutate_all(~replace(., is.na(.), 0))
# 
cor18bio <- cor(bio_18_cor, method = "spearman")
## r2
corrplot(cor18bio, type = "lower", order = "hclust", tl.col = "black", 
         tl.srt = 45,  addCoef.col="grey20", number.cex = .6,
         col = bb, diag=FALSE)
#  save    correlation_bio_18s_pearson_percent_log
pdf(paste(dir, export_cor_plot,'correlation_bio_18s_spearman.pdf',sep = ""), width=11, height=10 )
corrplot(cor18bio, type = "lower", order = "hclust", tl.col = "black", 
         tl.srt = 45, addCoef.col="grey20", number.cex = .6,
         col = bb, diag=FALSE)
dev.off()
...
bio_metag_cor<-dat_genome2_bio2 %>%   # names(DNA_net)
  filter(filter_size=="0.2.3") %>%  # unique(dat_genome2_bio2$filter_size)
select_at(c(5:13,18)) %>% 
mutate_all(~replace(., is.na(.), 0))  #%>% 
 # mutate_all(trans)
# plot(bio_metag_cor$metagenome_ec3_PID_70,bio_metag_cor$net_biomass)  names(bio_metag_cor)
#  , method = "spearman" 
cormetagbio <- cor(bio_metag_cor, method = "spearman")
## r2
corrplot(cormetagbio, type = "lower", order = "hclust", tl.col = "black", 
         tl.srt = 45, addCoef.col="grey20", number.cex = .6,
          diag=FALSE)  # col = bb,
##  11 samples ec3 pid_70 - 0.48, ec3_50-0.42, pf7_50 - 0.41
#  save    correlation_bio_metag_pearson_percent_log
pdf(paste(dir, export_cor_plot,'correlation_bio_metag_spearman_smallfilt.pdf',sep = ""), width=11, height=10 )
corrplot(cormetagbio*cormetagbio, type = "lower", order = "hclust", tl.col = "black", 
         tl.srt = 45, addCoef.col="grey20", number.cex = .6,
         col = bb, diag=FALSE)
dev.off()
...
datp<- DNA18s_ab2 %>%   #   names(DNA18s_ab2)
  ...
  mutate(Taxa= abund_taxa, ind = sqrt(net_abundance*100) , resp=sqrt(metabarcode_PID_80*100), fac_col=factor(paste(Station.x,Depth))) 
  
cor1 <- (cor(datp$ind,datp$resp, method = "spearman" ))^2
...
p_18s_abund<-ggplot(datp, aes(x = ind, y = resp)) +
  ...
  annotate("text", x = 5, y = 6, parse = TRUE, label = as.character(expression(r^{2}*"=0.10"))) +
  annotate("text", x = 5, y = 5, label = "n=41") +
...
datp<- DNA18s_bio2 %>%   #   names(DNA18s_bio2)
    mutate(Taxa= biomass_taxa, ind = sqrt(net_biomass*100) , resp=sqrt(metabarcode_PID_80*100), fac_col=factor(paste(Station.x,Depth))) 
...
p_18s_bio<-ggplot(datp, aes(x = ind, y = resp)) +
  ...
  scale_shape(solid=FALSE) +
  stat_smooth(method = "lm", col = "black", se = FALSE) +
  annotate("text", x = 2.5, y = 3.0, parse = TRUE, label = as.character(expression(r^{2}*"=0.22"))) +
  annotate("text", x = 2.5, y = 2, label = "n=23") +
...
datp<- dat_genome2_ab2 %>%   #   names(dat_genome2_ab2)
  mutate(abund_taxa =forcats::fct_recode(abund_taxa, Mollusca = "Pteropods", Polychaeta="Polychaets",  Chordata="Chordates")) %>%
    mutate(Taxa= abund_taxa, ind = sqrt(net_abundance*100) , resp=sqrt(metagenome_ec4_PID_70*100), fac_col=factor(paste(Station.x,Depth))) %>%
  filter(filter_size=="0.2.0.8"  | filter_size=="0.2.3")    # unique(datp$filter_size)

mes<-datp %>% 
    filter(!duplicated(fac_col))
#  get r2 and slope for each taxa group
nd<-datp %>%  # names(datp)  use Taxa
  group_by(Taxa) %>% 
  nest() %>% 
  mutate(fit = map(data, ~lm(metagenome_ec4_PID_70 ~ net_abundance, data = .)),
...
p_gen_ab_filt_small<-ggplot(datp, aes(x = ind, y = resp)) +
  ...
  scale_shape(solid=FALSE) +
  stat_smooth(method = "lm", col = "black", se = FALSE) +
  annotate("text", x = 2, y = 9, parse = TRUE, label = as.character(expression(r^{2}*"=0.73"))) +
  annotate("text", x = 2.0, y = 8.2, label = "n=13") +
...
p_gen_ab_filt_big<-ggplot(datp, aes(x = ind, y = resp)) +
  ...
  scale_shape(solid=FALSE) +
  stat_smooth(method = "lm", col = "black", se = FALSE) +
  annotate("text", x = 5, y = 9, parse = TRUE, label = as.character(expression(r^{2}*"=0.22"))) +
    annotate("text", x = 5, y = 8.2, label = "n=8") +
  labs(y = "metagenome (% sequences)", x = "net (% abundance)")  +
  theme_bw()  + # classic -no grids or bw or minimal
  guides(colour=FALSE) +
  #theme(legend.position="none") +
...
datp<- dat_genome2_bio2 %>%   #   names(dat_genome2_bio2)
    mutate(Taxa= biomass_taxa, ind = sqrt(net_biomass*100) , resp=sqrt(metagenome_ec4_PID_70*100), fac_col=factor(paste(Station.x,Depth))) %>%
  filter(filter_size=="0.2.0.8"  | filter_size=="0.2.3")    # unique(datp$filter_size)
...
nd<-datp %>%  # names(datp)  use Taxa
  group_by(Taxa) %>% 
  nest() %>% 
  mutate(fit = map(data, ~lm(metagenome_ec4_PID_70 ~ net_biomass, data = .)),
         summary = map(fit, broom::glance)) %>% 
  unnest(summary)
...
p_gen_bio_filt_small<-ggplot(datp, aes(x = ind, y = resp)) +
  ...
  scale_shape(solid=FALSE) +
  stat_smooth(method = "lm", col = "black", se = FALSE) +
  annotate("text", x = 2., y = 9, parse = TRUE, label = as.character(expression(r^{2}*"=0.45"))) +
  annotate("text", x = 2., y = 8.2, label = "n=12") +
...
p_gen_bio_filt_big<-ggplot(datp, aes(x = ind, y = resp)) +
  ...
  scale_shape(solid=FALSE) +
  stat_smooth(method = "lm", col = "black", se = FALSE) +
  annotate("text", x = 2., y = 2.8, parse = TRUE, label = as.character(expression(r^{2}*"=0.75"))) +
    annotate("text", x = 2., y = 2., label = "n=8") +
  labs(y = "metagenome (% sequences)", x = "net (% biomass)")  +
  theme_bw()  + # classic -no grids or bw or minimal
  guides(colour=FALSE) +
  #theme(legend.position="none") +
...
# all 6  ###################
plot6<-ggpubr::ggarrange(p_18s_abund, p_18s_bio, p_gen_ab_filt_small, p_gen_bio_filt_small, p_gen_ab_filt_big, p_gen_bio_filt_big  , ncol=2, nrow=3, labels=list("A","D","B","E","C","F"), font.label = list(face="plain", size=10)  )
#  export
  ggsave(filename=paste(dir,"Documents/KAUST/eDNA/DMAP/R/plots/comparison_2019/DMAP_6_correlations_rank.pdf",sep=""), plot = plot6, width = 30, height = 25, units = c("cm") )
...
#  export
ggsave(filename=paste(dir,"Documents/KAUST/eDNA/DMAP/R/plots/comparison_2019/net_gen_small_sf_correlations_rank.pdf",sep=""), plot = small, width=20 , height=7, units=c("cm"))
...
#  export
 ggsave(filename=paste(dir,"Documents/KAUST/eDNA/DMAP/R/plots/comparison_2019/net_gen_large_sf_correlations_rank.pdf",sep=""), plot = big , width=5.5 , height=8, units=c("in"))
...
#  export
#  ggsave(filename=paste(dir,"Documents/KAUST/eDNA/DMAP/R/plots/comparison_2019/net_gen_ec3_spearorrelations.pdf",sep=""), plot = a , width=8 , height=5, units=c("in"))

# genome small  ###################
small<-ggpubr::ggarrange(p_gen_ab_filt_small, p_gen_bio_filt_small, ncol=2, nrow=1, labels=list("A","B"), font.label = list(face="plain", size=10)  )
#  export
#  ggsave(filename=paste(dir,"Documents/KAUST/eDNA/DMAP/R/plots/comparison_2019/net_gen_small_sf_spearcorrelations.pdf",sep=""), plot = small, width=20 , height=7, units=c("cm"))

# genome big  ###################
big<-ggpubr::ggarrange(p_gen_ab_filt_big, p_gen_bio_filt_big  , ncol=1, nrow=2, labels=list("A","B"), font.label = list(face="plain", size=10)  )
#  export
#  ggsave(filename=paste(dir,"Documents/KAUST/eDNA/DMAP/R/plots/comparison_2019/net_gen_large_sf_spearcorrelations.pdf",sep=""), plot = big , width=5.5 , height=8, units=c("in"))
